...
string* process(string* request){
    ...
    if (space_counter == 2 && line_break_counter >= 2){
        ...
        if (str_start_with_chars(req->uri, "/debug", strlen("/debug"))){
            response_str = calloc(1, sizeof(string));
            const char* new_line = "\r\n";
            str_cat(response_str, req->method->str, req->method->len);
            str_cat(response_str, new_line, strlen(new_line));
            str_cat(response_str, req->uri->str, req->uri->len);
            str_cat(response_str, new_line, strlen(new_line));
            if (req->protocol != NULL){
                str_cat(response_str, req->protocol->str, req->protocol->len);
                str_cat(response_str, new_line, strlen(new_line));
            }

            string* file_path = char_to_string("../resources/debug.html");
            string* file = read_file_into_string(file_path->str, (unsigned int)file_path->len);

            char* tmp = get_nullterminated_char_str(req->method);
            str_replace_with(file, "${method}", tmp);
            free(tmp);
            tmp = get_nullterminated_char_str(req->uri);
            str_replace_with(file, "${ressource}", tmp);
            free(tmp);
            tmp = get_nullterminated_char_str(req->protocol);
            str_replace_with(file, "${version}", tmp);
            free(tmp);

            set_response_status(resp, char_to_string("200"), char_to_string("OK"));
            set_response_body(resp, file, char_to_string("text/html; charset=UTF-8"));

            str_free(response_str);
            str_free(file_path);
        }
        else if (str_cmp(req->method, get) == 0 && req->uri->len > 0 &&
            str_start_with_chars(req->uri, "/", strlen("/")) &&
            req->uri->len < 256){
            //Get File
            string* file_path;
            if (req->uri->len == 1){
                file_path = char_to_string("/index.html");
            }
            else{
                file_path = str_cpy(req->uri->str, req->uri->len);
            }
            string* file;
            ...
            case 1: //File exists
                ...
                if (file == NULL){//Filepath is directory, not a file
